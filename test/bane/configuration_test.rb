...
class ConfigurationTest < Test::Unit::TestCase
  ...
  def test_should_map_single_port_and_server_name
    ...
    assert_configuration(actual_entry, :port => 3000, :behavior => Bane::Behaviors::CloseAfterPause)
  end

  def test_should_map_multiple_servers_given_one_starting_port
    configuration = Bane::Configuration.new(3000, "CloseAfterPause", "CloseImmediately")
    assert_equal 2, configuration.to_a.size, "Should have created two configurations"

    assert_configuration(configuration.to_a[0], :port => 3000, :behavior => Bane::Behaviors::CloseAfterPause)
    assert_configuration(configuration.to_a[1], :port => 3001, :behavior => Bane::Behaviors::CloseImmediately)
  end
  ...
  def test_should_ask_service_registry_for_all_behaviors_if_none_specified
    ...
    configuration = Bane::Configuration.new(3000)
    assert_equal 2, configuration.to_a.size, "Should have created two configurations"
