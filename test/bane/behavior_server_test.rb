...
class BehaviorServerTest < Test::Unit::TestCase
  ...
  def test_initializes_server_on_specified_port
    server = DelegatingGServer.new(6000, IRRELEVANT_BEHAVIOR, 'hostname')
    assert_equal 6000, server.port
  end

  def test_initializes_server_on_specified_hostname
    server = DelegatingGServer.new(IRRELEVANT_PORT, IRRELEVANT_BEHAVIOR, 'hostname')
  ...
  def test_delegates_serve_call_to_behavior
    io = mock
    behavior = mock
    server = DelegatingGServer.new(IRRELEVANT_PORT, behavior, 'hostname')
  ...
  def assert_log_message_uses_short_behavior_name_for(method)
    logger = StringIO.new
    server = DelegatingGServer.new(IRRELEVANT_PORT, Bane::Behaviors::SampleForTesting.new, 'host')
